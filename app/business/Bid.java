    ...
    public long getUpId() {
        if (0 == this.upId) {
            ...
            switch (this.upNextFlag) {
                case 2:
                    sql += " and status in(1, 2) and amount > has_invested_amount";
                    break;
                case 3:
                    sql += " and status in(1, 2) and amount = has_invested_amount";
                    break;
                ...
                case 7:
                    sql = sql2;
                    sql += " and a.status = 4 and b.overdue_mark in(-1, -2, -3)";
                    break;
                case 9:
                    sql += " and status <= -1 and status >= -10";
                    break;
                case 10:
                    sql = sql2;
                    sql += " and a.status in(4, 5) and b.overdue_mark = -3";
                    break;
			
			/* 账单催收，我的账单 */
                case 11:
                    sql = sql2;
                    sql += " and year(b.repayment_time)= year(now()) and month(b.repayment_time)= month(now()) and b.overdue_mark not in(- 2, - 3) and a.manage_supervisor_id = " + this.manageSupervisorId;
                    break;
                case 12:
                    sql = sql2;
                    sql += " and b.overdue_mark in(-1, -2) and a.manage_supervisor_id = " + this.manageSupervisorId;
                    break;
                case 13:
                    sql = sql2;
                    sql += " and b.status in (0, -3) and a.manage_supervisor_id = " + this.manageSupervisorId;
                    break; 
			/* 账单催收，部门的账单 */
                case 14:
                    sql = sql2;
                    sql += " and year(b.repayment_time)= year(now()) and month(b.repayment_time)= month(now()) and b.overdue_mark not in(- 2, - 3) and a.manage_supervisor_id = 0";
                    break;
                case 15:
                    sql = sql2;
                    sql += " and b.overdue_mark in(-1, -2) and a.manage_supervisor_id = 0";
                    break;
                case 16:
                    sql = sql2;
                    sql += " and b.status in (0, -3) and a.manage_supervisor_id = 0";
                    break; 
			/* 账单催收，我的标 */
                case 106:
                case 108:
                    sql += " and status = " + this.status + " and manage_supervisor_id = " + this.manageSupervisorId;
                    break;
                case 110:
                    sql = sql2;
                    sql += " and a.status in(4, 5) and b.overdue_mark = -3 and a.manage_supervisor_id = " + this.manageSupervisorId;
                    break; 
			/* 账单催收，部门的标 */
                case 206:
                case 208:
                    sql += " and status = " + this.status + " and manage_supervisor_id = 0";
                    break;
                case 210:
                    sql = sql2;
                    sql += " and a.status in(4, 5) and b.overdue_mark = -3 and a.manage_supervisor_id = 0";
                    break;
                case 211:
                    sql += " and manage_supervisor_id > 0";
                    break;
			...
			/* 财务 */
                ...
                case 401:
                    sql = sql2;
                    sql += " and b.status in (-1, -2) and b.overdue_mark in (-1, -2, -3)";
                    break; //401.逾期账单列表。
                case 402:
                    sql = sql2;
                    sql += " and b.status in (0, -3)";
                    break; //402.已收款借款账单列表。
                case 403:
                    sql += " and status = 5";
                    break; //403.已完成借款标列表。
                case 500:
                    sql = sql3;
                    sql += " and b.status in (-1, -2, -5, -6)";
                    break; //500.待付款理财账单列表。
                case 501:
                    sql = sql3;
                    sql += " and b.status in (-2)";
                    break; //501.逾期未付理财账单列表。
                case 502:
                    sql = sql3;
                    sql += " and b.status in (-4, 0)";
                    break; //502.已付款理财账单列表。
                case 503:
                    sql = sql3;
                    sql += " and b.status in (-3)";
                    break; //503.本金垫付理财账单列表。
                case 504:
                    sql = sql2;
                    sql += " and a.status in(4, 5) and b.overdue_mark = -3";
                    break; //504.坏账借款标列表。
    ...
    public int getCountUpId() {
        if (0 == this.countUpId) {
			...
			/* 账单催收，部门的标 */
                ...
                case 210:
                    sql = sql2;
                    sql += " and a.status in(4, 5) and b.overdue_mark = -3 and a.manage_supervisor_id mkmk  = 0";
                    break;
                case 211:
                    sql += " and manage_supervisor_id > 0";
                    break;
