...
//            recoName = Encrypt.decrypt3DES(recommendUserName, Constants.ENCRYPTION_KEY);
        ...
        //invoke fp register function that create the record for table "c_authentication" and "c_customer"
        try {
            ...
            if (value != null && value instanceof JSONObject) {
                ...
                flash.put("authentication_id", authentication_id);
            }
        ...
        try {
            ...
            if (message != null && message instanceof JSONObject) {
                ...
                if (!severity.equals("0")) {
                    error.code = -1;
                    error.msg = ((JSONObject) message).getString("summary");
                    flash.error("在FP中" + error.msg + "， 送金豆失败！");
                }
            }
            Logger.info("注册送金豆接口:statusCode-----------:" + statusCode + ", result----------:" + result);
        } catch (HttpException e) {
            e.printStackTrace();
            flash.error("FP注册成功,送金豆失败,请联系客服！");
            register();
        } catch (IOException e) {
            e.printStackTrace();
            flash.error("FP注册成功,送金豆失败,请联系客服！");
